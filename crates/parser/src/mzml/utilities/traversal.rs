...
macro_rules! impl_param_collector {
    ...
    ($ty:ty { cv: $cv:ident, user: $user:ident, sw: $sw:ident, ref: $r:ident }) => {
        impl ParamCollector for $ty {
            fn receive_cv(&mut self, p: CvParam) {
                self.$cv.push(p);
            }
...
impl_param_collector!(ScanList         { cv: cv_params, user: user_params, ref: referenceable_param_group_refs      });
