...
pub(crate) static ALP2: Alp2 = Alp2;
pub(crate) struct Alp2;

impl Packing for Alp2 {
    ...
    fn encode(&self, input: PackingInput<'_>, out: &mut Vec<u8>) -> IonResult<()> {
        match input {
            ...
            _ => Err(IonError::from("ALP2 requires F32 or F64 input")),
        }
    }
    ...
    fn decode(&self, input: &[u8], dtype: Dtype, out: &mut Vec<u8>) -> IonResult<()> {
        match dtype {
            ...
            _ => Err(IonError::from("ALP2: unsupported dtype")),
        }
    }
}
...
fn encode_f64(values: &[f64], out: &mut Vec<u8>) -> IonResult<()> {
    let n = u32::try_from(values.len())
        .map_err(|_| IonError::from("ALP2 F64: input exceeds u32 element count"))?;

    let (exp, encoded, patch_idx, patch_val, _chunk_offsets) = f64::encode(values, None);
    let patch_count = u32::try_from(patch_idx.len())
        .map_err(|_| IonError::from("ALP2 F64: patch count exceeds u32"))?;
...
fn decode_f64(input: &[u8], out: &mut Vec<u8>) -> IonResult<()> {
    if input.len() < HDR {
        return Err(IonError::from("ALP2 F64: truncated header"));
    }
    ...
    let encoded_end = HDR
        .checked_add(n.checked_mul(8).ok_or_else(|| IonError::from("ALP2 F64: encoded size overflow"))?)
        .ok_or_else(|| IonError::from("ALP2 F64: encoded offset overflow"))?;
    let patch_idx_end = encoded_end
        .checked_add(patch_count.checked_mul(4).ok_or_else(|| IonError::from("ALP2 F64: patch index overflow"))?)
        .ok_or_else(|| IonError::from("ALP2 F64: patch index offset overflow"))?;
    let patch_val_end = patch_idx_end
        .checked_add(patch_count.checked_mul(8).ok_or_else(|| IonError::from("ALP2 F64: patch value overflow"))?)
        .ok_or_else(|| IonError::from("ALP2 F64: patch value offset overflow"))?;

    if input.len() < patch_val_end {
        return Err(IonError::from("ALP2 F64: truncated data"));
    }
    ...
    for i in 0..patch_count {
        ...
        if idx >= n {
            return Err(IonError::from("ALP2 F64: patch index out of bounds"));
        }
...
fn encode_f32(values: &[f32], out: &mut Vec<u8>) -> IonResult<()> {
    let n = u32::try_from(values.len())
        .map_err(|_| IonError::from("ALP2 F32: input exceeds u32 element count"))?;

    let (exp, encoded, patch_idx, patch_val, _chunk_offsets) = f32::encode(values, None);
    let patch_count = u32::try_from(patch_idx.len())
        .map_err(|_| IonError::from("ALP2 F32: patch count exceeds u32"))?;
...
fn decode_f32(input: &[u8], out: &mut Vec<u8>) -> IonResult<()> {
    if input.len() < HDR {
        return Err(IonError::from("ALP2 F32: truncated header"));
    }
    let exp = Exponents { e: input[0], f: input[1] };
    let n = u32::from_le_bytes(input[2..6].try_into().unwrap()) as usize;
    let patch_count = u32::from_le_bytes(input[6..10].try_into().unwrap()) as usize;
    ...
    let encoded_end = HDR
        .checked_add(n.checked_mul(4).ok_or_else(|| IonError::from("ALP2 F32: encoded size overflow"))?)
        .ok_or_else(|| IonError::from("ALP2 F32: encoded offset overflow"))?;
    let patch_idx_end = encoded_end
        .checked_add(patch_count.checked_mul(4).ok_or_else(|| IonError::from("ALP2 F32: patch index overflow"))?)
        .ok_or_else(|| IonError::from("ALP2 F32: patch index offset overflow"))?;
    let patch_val_end = patch_idx_end
        .checked_add(patch_count.checked_mul(4).ok_or_else(|| IonError::from("ALP2 F32: patch value overflow"))?)
        .ok_or_else(|| IonError::from("ALP2 F32: patch value offset overflow"))?;

    if input.len() < patch_val_end {
        return Err(IonError::from("ALP2 F32: truncated data"));
    }
    ...
    for i in 0..patch_count {
        ...
        if idx >= n {
            return Err(IonError::from("ALP2 F32: patch index out of bounds"));
        }
...
mod tests {
    ...
    fn roundtrip_f64(vals: &[f64]) -> Vec<f64> {
        let mut enc = Vec::new();
        ALP2.encode(PackingInput::F64(vals), &mut enc).unwrap();
        let mut dec = Vec::new();
        ALP2.decode(&enc, Dtype::F64, &mut dec).unwrap();
        dec.chunks_exact(8).map(|c| f64::from_le_bytes(c.try_into().unwrap())).collect()
    }

    fn roundtrip_f32(vals: &[f32]) -> Vec<f32> {
        let mut enc = Vec::new();
        ALP2.encode(PackingInput::F32(vals), &mut enc).unwrap();
        let mut dec = Vec::new();
        ALP2.decode(&enc, Dtype::F32, &mut dec).unwrap();
    ...
    fn wrong_input_type_returns_error() {
        let mut out = Vec::new();
        assert!(ALP2.encode(PackingInput::I32(&[1, 2, 3]), &mut out).is_err());
    }

    #[test]
    fn rejects_truncated_header() {
        let mut out = Vec::new();
        assert!(ALP2.decode(&[0u8; 4], Dtype::F64, &mut out).is_err());
        assert!(ALP2.decode(&[0u8; 4], Dtype::F32, &mut out).is_err());
    }
}
