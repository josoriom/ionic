...
impl Packing for DeltaShuffle {
    ...
    fn encode(&self, input: PackingInput<'_>, out: &mut Vec<u8>) -> IonResult<()> {
        match input {
            ...
            _ => Err(IonError::from("DeltaShuffle requires F32 or F64 input")),
        }
    }
...
mod tests {
    ...
    fn roundtrip_f64(input: &[f64]) -> Vec<f64> {
        let mut enc = Vec::new();
        DELTA_SHUFFLE.encode(PackingInput::F64(input), &mut enc).unwrap();
        let mut dec = Vec::new();
    ...
    fn roundtrip_f32(input: &[f32]) -> Vec<f32> {
        let mut enc = Vec::new();
        DELTA_SHUFFLE.encode(PackingInput::F32(input), &mut enc).unwrap();
        let mut dec = Vec::new();
    ...
    #[test]
    fn single_element_f32_is_bit_exact() {
        let input = [503.42f32];
    ...
    #[test]
    fn two_elements_f32_are_bit_exact() {
        let input = [100.0f32, 200.0];
        let got = roundtrip_f32(&input);
        for (a, b) in got.iter().zip(input.iter()) {
            assert_eq!(a.to_bits(), b.to_bits());
        }
    ...
    #[test]
    fn encoded_length_f32_equals_input_bytes() {
