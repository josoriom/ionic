...
pub(crate) const HEADER_SPEC_SUMMARY_CRC32: usize = 424;
pub(crate) const HEADER_SPEC_ENTRIES_CRC32: usize = 428;
pub(crate) const HEADER_SPEC_ARRAYREFS_CRC32: usize = 432;
pub(crate) const HEADER_CHROM_SUMMARY_CRC32: usize = 436;
pub(crate) const HEADER_CHROM_ENTRIES_CRC32: usize = 440;
pub(crate) const HEADER_CHROM_ARRAYREFS_CRC32: usize = 444;

pub(crate) const RESERVED_BLOCK_START: usize = 448;
pub(crate) const RESERVED_BLOCK_SIZE: usize = 544;
...
const _: () = assert!(HEADER_SPEC_SUMMARY_CRC32 == 424);
const _: () = assert!(HEADER_SPEC_ENTRIES_CRC32 == 428);
const _: () = assert!(HEADER_SPEC_ARRAYREFS_CRC32 == 432);
const _: () = assert!(HEADER_CHROM_SUMMARY_CRC32 == 436);
const _: () = assert!(HEADER_CHROM_ENTRIES_CRC32 == 440);
const _: () = assert!(HEADER_CHROM_ARRAYREFS_CRC32 == 444);
...
pub(crate) struct Header {
    ...
    pub(crate) spec_arrayrefs_crc32: u32,
...
impl Default for Header {
    fn default() -> Self {
        Self {
            ...
            off_spec_entries: 0,
            len_spec_entries: 0,
            off_spec_arrayrefs: 0,
            len_spec_arrayrefs: 0,
            off_chrom_entries: 0,
            len_chrom_entries: 0,
            off_chrom_arrayrefs: 0,
            len_chrom_arrayrefs: 0,
            off_spec_meta: 0,
            len_spec_meta: 0,
            off_chrom_meta: 0,
            len_chrom_meta: 0,
...
impl Header {
    pub(crate) fn parse(bytes: &[u8]) -> IonResult<Self> {
        ...
        let spec_arrayrefs_crc32 = read_u32_at(h, HEADER_SPEC_ARRAYREFS_CRC32);
        ...
        if reserved.iter().any(|&b| b != 0) {
            return Err("header: reserved block 448..992 must be all zeros".into());
        }
    ...
    pub(crate) fn write(&self, buf: &mut [u8]) {
        ...
        write_u64_at(buf, HEADER_OFFSET_SPEC_ARRAYREFS, self.off_spec_arrayrefs);
        write_u64_at(buf, HEADER_LEN_SPEC_ARRAYREFS, self.len_spec_arrayrefs);
        ...
        write_u64_at(buf, HEADER_OFFSET_CHROM_ARRAYREFS, self.off_chrom_arrayrefs);
        write_u64_at(buf, HEADER_LEN_CHROM_ARRAYREFS, self.len_chrom_arrayrefs);
        ...
        write_u32_at(buf, HEADER_SPEC_ARRAYREFS_CRC32, self.spec_arrayrefs_crc32);
...
pub(crate) fn validate_file_integrity(bytes: &[u8], h: &Header) -> (bool, Vec<String>) {
    ...
    let sections: &[(&str, u64, u64)] = &[
        ...
        ("chrom_arrayrefs", h.off_chrom_arrayrefs, h.len_chrom_arrayrefs),
...
mod tests {
    ...
    fn header_offsets_match_clean_v1_layout() {
        ...
        assert_eq!(HEADER_SPEC_SUMMARY_CRC32, 424);
        assert_eq!(HEADER_SPEC_ENTRIES_CRC32, 428);
        assert_eq!(HEADER_SPEC_ARRAYREFS_CRC32, 432);
        assert_eq!(HEADER_CHROM_SUMMARY_CRC32, 436);
        assert_eq!(HEADER_CHROM_ENTRIES_CRC32, 440);
        assert_eq!(HEADER_CHROM_ARRAYREFS_CRC32, 444);
        assert_eq!(RESERVED_BLOCK_START, 448);
        assert_eq!(RESERVED_BLOCK_SIZE, 544);
    ...
    fn write_then_parse_round_trips_every_field() {
        let original = Header {
            ...
            off_spec_arrayrefs: 0x0016_0000_0000_0006,
            len_spec_arrayrefs: 0x0017_0000_0000_0007,
            off_chrom_entries: 0x0018_0000_0000_0008,
            len_chrom_entries: 0x0019_0000_0000_0009,
            off_chrom_arrayrefs: 0x001a_0000_0000_000a,
            len_chrom_arrayrefs: 0x001b_0000_0000_000b,
            ...
            spec_arrayrefs_crc32: 0xAABB_CC0C,
        ...
        assert_eq!(parsed.off_spec_arrayrefs, original.off_spec_arrayrefs);
        assert_eq!(parsed.len_spec_arrayrefs, original.len_spec_arrayrefs);
        ...
        assert_eq!(parsed.off_chrom_arrayrefs, original.off_chrom_arrayrefs);
        assert_eq!(parsed.len_chrom_arrayrefs, original.len_chrom_arrayrefs);
        ...
        assert_eq!(parsed.spec_arrayrefs_crc32, original.spec_arrayrefs_crc32);
