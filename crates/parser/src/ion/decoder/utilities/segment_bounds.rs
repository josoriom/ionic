...
pub(crate) struct SegmentBoundsIndex {
    rows: Vec<SegmentBound>,
}

impl SegmentBoundsIndex {
    ...
    pub(crate) fn from_bytes(bytes: &[u8], spec_array_ref_count: u64) -> IonResult<Self> {
        if !bytes.len().is_multiple_of(SEGMENT_BOUND_SIZE) {
            return Err("segment bounds: plain_length not divisible by 24".into());
        }

        let row_count = bytes.len() / SEGMENT_BOUND_SIZE;
        let mut rows: Vec<SegmentBound> = Vec::with_capacity(row_count);
        ...
        for i in 0..row_count {
            ...
            if i > 0 && rows[i - 1].array_ref_index >= array_ref_index {
                return Err("segment bounds: keys not strictly ascending".into());
            }
            if array_ref_index >= spec_array_ref_count {
                return Err("segment bounds: array_ref_index out of range".into());
            }
            if !low.is_finite() || !high.is_finite() || low > high {
                return Err("segment bounds: invalid low/high value".into());
            }

            rows.push(SegmentBound {
                array_ref_index,
                low,
                high,
            });
...
mod tests {
    ...
    fn from_bytes_parses_sorted_rows() {
        ...
        let index = SegmentBoundsIndex::from_bytes(&bytes, 4).unwrap();
    ...
    fn from_bytes_rejects_unaligned_length() {
        ...
        assert!(SegmentBoundsIndex::from_bytes(&bytes, 4).is_err());
    }
    ...
    fn from_bytes_rejects_non_ascending_keys() {
        ...
        assert!(SegmentBoundsIndex::from_bytes(&bytes, 8).is_err());
    }

    #[test]
    fn from_bytes_rejects_index_out_of_range() {
        let bytes = row_bytes(5, 1.0, 2.0);
        assert!(SegmentBoundsIndex::from_bytes(&bytes, 5).is_err());
    }
    ...
    fn from_bytes_rejects_non_finite_values() {
        ...
        assert!(SegmentBoundsIndex::from_bytes(&nan, 4).is_err());
        let inf = row_bytes(0, 1.0, f64::INFINITY);
        assert!(SegmentBoundsIndex::from_bytes(&inf, 4).is_err());
    }

    #[test]
    fn get_returns_none_on_empty() {
        let index = SegmentBoundsIndex::from_bytes(&[], 0).unwrap();
