...
#[test]
fn assign_attrs_spectrum_list_ion_equiv_schema_subset() {
    ...
    let ion_meta = spectra_meta_from_test_ion();
...
#[test]
fn assign_attrs_spectrum_first_ion_equiv_schema_subset() {
    let mzml = parse_mzml_cached(&MZML_CACHE, MZML_PATH);
    let sl = mzml
        .run
        .spectrum_list
    ...
    let id = find_id_by_tail_text(&ion_meta, TagId::Spectrum, ACC_ATTR_ID, s0.id.as_str());
...
#[test]
fn assign_attrs_spectrum_last_ion_equiv_schema_subset() {
    let mzml = parse_mzml_cached(&MZML_CACHE, MZML_PATH);
    let sl = mzml
        .run
        .spectrum_list
    ...
    let id = find_id_by_tail_text(&ion_meta, TagId::Spectrum, ACC_ATTR_ID, s_last.id.as_str());
