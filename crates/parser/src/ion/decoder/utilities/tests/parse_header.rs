use crate::{
    encoder::encode::{CHROM_FILTER_RECORD_SIZE, SPEC_FILTER_RECORD_SIZE},
    ion::utilities::parse_header,
};
...
fn check_header() {
    ...
    assert_eq!(
        header.len_spec_filter,
        header.spectrum_count * SPEC_FILTER_RECORD_SIZE as u64
    );
    assert_eq!(
        header.len_chrom_filter,
        header.chrom_count * CHROM_FILTER_RECORD_SIZE as u64
    );
    ...
    // section disk order: spec_filter → spec_entries → spec_arrayrefs →
    //                     chrom_filter → chrom_entries → chrom_arrayrefs →
    //                     spec_meta → chrom_meta → global_meta
    assert!(
        header
            .off_spec_filter
