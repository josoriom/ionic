...
mod tests {
    ...
    fn parse_global_metadata_rejects_section_that_is_too_small() {
        let tiny = vec![0u8; 4];
        let result = parse_global_metadata(&tiny, 0, 0, 0, 0, HDR_CODEC_NONE, 0, DecompressionBudget::default());
    ...
    fn parse_global_metadata_rejects_zero_counts_in_header() {
        ...
        let result = parse_global_metadata(&bytes, 0, 0, 0, 0, HDR_CODEC_NONE, 0, DecompressionBudget::default());
        assert!(result.is_err());
    ...
    fn parse_global_metadata_rejects_item_count_mismatch() {
        ...
        let result = parse_global_metadata(&bytes, 99, 0, 0, 0, HDR_CODEC_NONE, 0, DecompressionBudget::default());
        assert!(result.is_err());
    ...
    fn parse_global_metadata_rejects_unsupported_codec() {
        ...
        let result = parse_global_metadata(&bytes, 0, 0, 0, 0, 99, 0, DecompressionBudget::default());
        assert!(result.is_err());
    ...
    fn parse_global_metadata_rejects_trailing_nonzero_bytes_for_uncompressed() {
        ...
        let result = parse_global_metadata(&bytes, 0, 0, 0, 0, HDR_CODEC_NONE, expected as u64, DecompressionBudget::default());
        assert!(result.is_err());
