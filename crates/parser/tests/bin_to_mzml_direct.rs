//! T2-5: Direct test of `convert_bin_to_mzml_bytes()` and `bin_to_mzml()`.
//!
//! Exercises the binary→mzML XML serialization path with synthetic data,
//! verifying that the output is valid XML that can be re-parsed into
//! equivalent MzML structs.

mod common;

use common::binary_ext::BinaryDataExt;
use common::builders::{
    default_cv_list_like_writer, minimal_file_description, synthetic_binary_data_array,
};
use ionic::mzml::{
...
/// Build a minimal but valid MzML with the given spectra and chromatograms.
...
fn make_spectrum(id: &str, mz: Vec<f64>, intensity: Vec<f64>) -> Spectrum {
    let len = mz.len();
    Spectrum {
        id: id.to_string(),
        index: Some(0),
...
fn make_chromatogram(id: &str, time: Vec<f64>, intensity: Vec<f64>) -> Chromatogram {
    let len = time.len();
    Chromatogram {
        id: id.to_string(),
        index: Some(0),
...
fn convert_bin_to_mzml_bytes_produces_valid_xml() {
    ...
    let mzml = build_mzml(
        vec![make_spectrum("scan=1", mz.clone(), intensity.clone())],
        vec![],
    );
    ...
    // Should contain mzML tags
    assert!(xml_str.contains("<mzML"), "should have <mzML> tag");
    assert!(xml_str.contains("</mzML>"), "should have closing </mzML> tag");
...
fn bin_to_mzml_string_version() {
    let mzml = build_mzml(
        vec![make_spectrum("scan=1", vec![1.0, 2.0], vec![10.0, 20.0])],
        vec![],
    );
...
fn roundtrip_preserves_mz_values() {
    ...
    let arrays = spectra.spectra[0]
        .binary_data_array_list
        .as_ref()
        .unwrap();

    // Find mz array (MS:1000514) and intensity array (MS:1000515)
    let mz_bda = arrays
        .binary_data_arrays
        .iter()
        .find(|a| {
    ...
    // Values should be exactly preserved (f64 → base64 → f64 is lossless)
    assert_eq!(got_mz, mz, "m/z values should be exactly preserved");
    assert_eq!(got_int, intensity, "intensity values should be exactly preserved");
}
...
fn roundtrip_with_chromatogram() {
    ...
    let mzml = build_mzml(
        vec![],
        vec![make_chromatogram("TIC", time.clone(), intensity.clone())],
    );
...
fn roundtrip_multiple_spectra() {
    let spectra = vec![
        make_spectrum("scan=1", vec![100.0, 200.0], vec![10.0, 20.0]),
        make_spectrum("scan=2", vec![150.0, 250.0], vec![15.0, 25.0]),
        make_spectrum("scan=3", vec![300.0], vec![30.0]),
    ];
    // Fix indices
