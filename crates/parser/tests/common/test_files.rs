// ── pwiz test_files ──────────────────────────────────────────────────────

test_file!(tiny_pwiz_10, "crates/parser/data/mzml/tiny.pwiz.1.0.mzML");
test_file!(tiny_pwiz_11, "crates/parser/data/mzml/tiny.pwiz.1.1.mzML");
test_file!(
    tiny_pwiz_111,
    "crates/parser/data/mzml/tiny.pwiz.1.1.1.mzML"
);
test_file!(tiny2_pwiz_10, "crates/parser/data/mzml/tiny2.pwiz.1.0.mzML");
test_file!(small_pwiz_10, "crates/parser/data/mzml/small.pwiz.1.0.mzML");
test_file!(small_pwiz_11, "crates/parser/data/mzml/small.pwiz.1.1.mzML");
test_file!(
    small_zlib_pwiz_11,
    "crates/parser/data/mzml/small_zlib.pwiz.1.1.mzML"
);
test_file!(
    small_miape_pwiz_11,
    "crates/parser/data/mzml/small_miape.pwiz.1.1.mzML"
);

// ── Internal parser test_files ───────────────────────────────────────────

test_file!(anpc_test_mzml, "crates/parser/data/mzml/test.mzML");
test_file!(
    legacy_pwiz_099_10,
    "crates/parser/data/mzml/tiny.pwiz.mzML0.99.10.mzML"
);
test_file!(
    legacy_pwiz_099_9,
    "crates/parser/data/mzml/tiny.pwiz.mzML0.99.9.mzML"
);
test_file!(
    legacy_msdata_099_10,
    "crates/parser/data/mzml/tiny.msdata.mzML0.99.10.mzML"
);
test_file!(
    legacy_msdata_099_9,
    "crates/parser/data/mzml/tiny.msdata.mzML0.99.9.mzML"
);
test_file!(
    legacy_tiny1_099_0,
    "crates/parser/data/mzml/tiny1.mzML0.99.0.mzML"
);
test_file!(
    legacy_tiny2_srm_099_0,
    "crates/parser/data/mzml/tiny2_SRM.mzML0.99.0.mzML"
);
test_file!(
    legacy_tiny2_srm_099_1,
    "crates/parser/data/mzml/tiny2_SRM.mzML0.99.1.mzML"
);
test_file!(
    legacy_tiny4_ltq_ft_099_0,
    "crates/parser/data/mzml/tiny4_LTQ-FT.mzML0.99.0.mzML"
);

/// All pwiz test_file paths for iteration.
pub const PWIZ_TEST_FILES: &[&str] = &[
    "crates/parser/data/mzml/tiny.pwiz.1.0.mzML",
    "crates/parser/data/mzml/tiny.pwiz.1.1.mzML",
    "crates/parser/data/mzml/tiny.pwiz.1.1.1.mzML",
    "crates/parser/data/mzml/tiny2.pwiz.1.0.mzML",
    "crates/parser/data/mzml/small.pwiz.1.0.mzML",
    "crates/parser/data/mzml/small.pwiz.1.1.mzML",
    "crates/parser/data/mzml/small_zlib.pwiz.1.1.mzML",
    "crates/parser/data/mzml/small_miape.pwiz.1.1.mzML",
];

/// All internal mzML test_file paths for iteration.
pub const INTERNAL_MZML_TEST_FILES: &[&str] = &[
    "crates/parser/data/mzml/test.mzML",
    "crates/parser/data/mzml/tiny.pwiz.mzML0.99.10.mzML",
    "crates/parser/data/mzml/tiny.pwiz.mzML0.99.9.mzML",
    "crates/parser/data/mzml/tiny.msdata.mzML0.99.10.mzML",
    "crates/parser/data/mzml/tiny.msdata.mzML0.99.9.mzML",
    "crates/parser/data/mzml/tiny1.mzML0.99.0.mzML",
    "crates/parser/data/mzml/tiny2_SRM.mzML0.99.0.mzML",
    "crates/parser/data/mzml/tiny2_SRM.mzML0.99.1.mzML",
    "crates/parser/data/mzml/tiny4_LTQ-FT.mzML0.99.0.mzML",
];

/// All test_file paths combined.
pub const ALL_TEST_FILES: &[&str] = &[
    "crates/parser/data/mzml/tiny.pwiz.1.0.mzML",
    "crates/parser/data/mzml/tiny.pwiz.1.1.mzML",
    "crates/parser/data/mzml/tiny.pwiz.1.1.1.mzML",
    "crates/parser/data/mzml/tiny2.pwiz.1.0.mzML",
    "crates/parser/data/mzml/small.pwiz.1.0.mzML",
    "crates/parser/data/mzml/small.pwiz.1.1.mzML",
    "crates/parser/data/mzml/small_zlib.pwiz.1.1.mzML",
    "crates/parser/data/mzml/small_miape.pwiz.1.1.mzML",
    "crates/parser/data/mzml/test.mzML",
    "crates/parser/data/mzml/tiny.pwiz.mzML0.99.10.mzML",
    "crates/parser/data/mzml/tiny.pwiz.mzML0.99.9.mzML",
    "crates/parser/data/mzml/tiny.msdata.mzML0.99.10.mzML",
    "crates/parser/data/mzml/tiny.msdata.mzML0.99.9.mzML",
    "crates/parser/data/mzml/tiny1.mzML0.99.0.mzML",
    "crates/parser/data/mzml/tiny2_SRM.mzML0.99.0.mzML",
    "crates/parser/data/mzml/tiny2_SRM.mzML0.99.1.mzML",
    "crates/parser/data/mzml/tiny4_LTQ-FT.mzML0.99.0.mzML",
];
