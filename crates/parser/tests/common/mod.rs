...
pub use binary_ext::BinaryDataExt;
...
pub fn decode_ion(bytes: &[u8]) -> Result<MzML, String> {
...
pub fn spectra(mzml: &MzML) -> &[Spectrum] {
    mzml.run
        .spectrum_list
        .as_ref()
...
pub fn chromatograms(mzml: &MzML) -> &[Chromatogram] {
    mzml.run
        .chromatogram_list
        .as_ref()
...
pub fn spectrum_arrays(s: &Spectrum) -> &[BinaryDataArray] {
...
pub fn chromatogram_arrays(c: &Chromatogram) -> &[BinaryDataArray] {
...
pub fn chromatogram_by_id<'a>(mzml: &'a MzML, id: &str) -> &'a Chromatogram {
    chromatograms(mzml)
        .iter()
        .find(|c| c.id == id)
...
pub fn find_array_by_accession<'a>(
...
pub fn cv_has_accession(cv_params: &[CvParam], accession: &str) -> bool {
    cv_params
        .iter()
        .any(|p| p.accession.as_deref() == Some(accession))
}

pub fn cv_param_by_accession<'a>(cv_params: &'a [CvParam], accession: &str) -> Option<&'a CvParam> {
    cv_params
        .iter()
        .find(|p| p.accession.as_deref() == Some(accession))
}

pub fn cv_value_f64(cv_params: &[CvParam], accession: &str) -> Option<f64> {
    cv_param_by_accession(cv_params, accession)
        .and_then(|p| p.value.as_deref())
        .and_then(|v| v.parse::<f64>().ok())
}

pub fn bda_role(bda: &BinaryDataArray) -> &'static str {
...
// ---------------------------------------------------------------------------
// Spectrum dual-path accessors
// ---------------------------------------------------------------------------

pub fn scan_list_of_spectrum(s: &Spectrum) -> Option<&ScanList> {
...
pub fn scan_list_of_spectrum_mut(s: &mut Spectrum) -> Option<&mut ScanList> {
...
pub fn precursor_list_of_spectrum(s: &Spectrum) -> Option<&PrecursorList> {
    if let Some(sd) = s.spectrum_description.as_ref()
...
pub fn product_list_of_spectrum(s: &Spectrum) -> Option<&ProductList> {
    if let Some(sd) = s.spectrum_description.as_ref()
...
pub fn first_scan(s: &Spectrum) -> &Scan {
    scan_list_of_spectrum(s)
        .and_then(|sl| sl.scans.first())
        .expect("first scan must exist")
}

pub fn first_scan_mut(s: &mut Spectrum) -> &mut Scan {
...
// ---------------------------------------------------------------------------
// Spectrum / chromatogram count helpers
// ---------------------------------------------------------------------------

pub fn spectrum_scan_count(s: &Spectrum) -> usize {
    if let Some(sd) = s.spectrum_description.as_ref() {
...
pub fn spectrum_precursor_count(s: &Spectrum) -> usize {
    if let Some(sd) = s.spectrum_description.as_ref() {
        return sd
            .precursor_list
            .as_ref()
...
pub fn spectrum_product_count(s: &Spectrum) -> usize {
    if let Some(sd) = s.spectrum_description.as_ref() {
        return sd
            .product_list
            .as_ref()
...
// ---------------------------------------------------------------------------
// ID navigation utilities
// ---------------------------------------------------------------------------

pub fn parse_scan_number_from_id(id: &str) -> Option<u32> {
    id.split_whitespace()
        .find_map(|tok| tok.strip_prefix("scan="))
        .and_then(|v| v.parse::<u32>().ok())
}

pub fn scan_start_time_seconds(s: &Spectrum) -> Option<f64> {
...
pub fn id_name_value_pairs(id: &str) -> Vec<(&str, &str)> {
    id.split_whitespace()
        .filter_map(|tok| tok.split_once('='))
        .collect()
}

pub fn find_name_value_indices(mzml: &MzML, key: &str, value: &str) -> Vec<usize> {
    spectra(mzml)
        .iter()
        .enumerate()
        .filter_map(|(i, s)| {
...
pub fn find_spot_id_indices(mzml: &MzML, spot_id: &str) -> Vec<usize> {
    spectra(mzml)
        .iter()
        .enumerate()
...
pub fn first_array_values_by_accession(s: &Spectrum, accession: &str) -> Vec<f64> {
...
pub fn first_chrom_array_values_by_accession(c: &Chromatogram, accession: &str) -> Vec<f64> {
    use binary_ext::BinaryDataExt;
...
// ---------------------------------------------------------------------------
// Set / ID collection utilities
// ---------------------------------------------------------------------------

pub fn set_of_ids<T, F>(items: &[T], mut f: F) -> BTreeSet<String>
where
...
pub fn top_level_software_ids(m: &MzML) -> BTreeSet<String> {
    m.software_list
        .as_ref()
...
pub fn top_level_dp_ids(m: &MzML) -> BTreeSet<String> {
...
pub fn top_level_source_file_ids(m: &MzML) -> BTreeSet<String> {
    m.file_description
        .as_ref()
...
pub fn top_level_instrument_ids(m: &MzML) -> BTreeSet<String> {
    m.instrument_list
        .as_ref()
...
pub fn top_level_sample_ids(m: &MzML) -> BTreeSet<String> {
    m.sample_list
        .as_ref()
...
pub fn hash_binary_payload(bin: &BinaryData) -> u64 {
    use binary_ext::BinaryDataExt;
    fnv64_bytes(&bin.to_le_bytes())
}

pub fn canonical_hash_index(mzml: &MzML) -> std::collections::BTreeMap<String, u64> {
...
pub fn canonical_diff_paths(left: &MzML, right: &MzML) -> Vec<String> {
...
pub fn semantic_fingerprint(mzml: &MzML) -> String {
